...
namespace WDBReader.WDBSchema
{
    sealed class QuestCacheMap : ClassMap<QuestCache>
    {
        public QuestCacheMap()
        {
            ...
            Map(m => m.RewardFactionID).Index(76, 80);
            Map(m => m.RewardFactionValue).Index(81, 85);
            Map(m => m.RewardFactionOverride).Index(86, 90);
            Map(m => m.RewardFactionGainMaxRank).Index(91, 95);
            Map(m => m.RewardFactionFlags);
            Map(m => m.RewardCurrencyID).Index(97, 100);
            Map(m => m.RewardCurrencyQuantity).Index(101, 104);
